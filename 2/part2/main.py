...
data = """Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
...
current_game_dict = {"blue": "-1", "red":"-1", "green":"-1"}
all_dicts = []
all_games = []
flagga = False
current_game = -1
...
game_ok = False
game_sum = 0
most_red =  most_blue =  most_green = 1
new_list = []
for game in all_games:
    numbers = []
    for dict in game:
        if int(dict["red"]) > most_red:
            most_red = int(dict["red"])
        if int(dict["blue"]) > most_blue:
            most_blue = int(dict["blue"])
        if int(dict["green"]) > most_green:
            most_green = int(dict["green"])
    numbers.append(most_green)
    numbers.append(most_red)
    numbers.append(most_blue)
    new_list.append(numbers)
    most_red =  most_blue =  most_green = 1
power = 1
for elem in new_list:
    power = elem[0] * elem[1] * elem[2]
    game_sum += power
print(game_sum)