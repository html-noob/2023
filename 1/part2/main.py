...
data_input = """7jlncfksix7rjgrpglmn9
...
def find_first_normal_digit(line):
    two_stuff = [9999, "penis"]
    dict = {}
    for char in line:
        if char.isdigit():
...
def find_last_normal_digit(line):
    two_stuff = [-1, "deck"]
    for char in line:
        if char.isdigit():
            current_latest_index = line.rfind(char)
            two_stuff[0] = current_latest_index
            two_stuff[1] = char
    return two_stuff
    
def find_first_string_digit(line):
    two_stuff = [9999, "penis"]
    #first val is index, second val is the number spelled out
    for digit in digits:
        index = line.find(digit)
        if index != -1 and index < two_stuff[0]:
            two_stuff[0] = index
            two_stuff[1] = digit
    return two_stuff
            
        
def find_last_string_digit(line):
    two_stuff = [-1, "deck"]
    #first val is index, second val is the number spelled out
    for digit in digits:
        index = line.rfind(digit)
        if index > two_stuff[0]:
            two_stuff[0] = index
            two_stuff[1] = digit
    return two_stuff
...
for line in data_input:
    #print(line)
    x = find_first_normal_digit(line)
    y = find_last_normal_digit(line)
    
    a = find_first_string_digit(line)
    b = find_last_string_digit(line)
    ...
    if not other_correct.isdigit():
        other_correct = digit_dict.get(other_correct)
    if not first_correct.isdigit():
        first_correct = digit_dict.get(first_correct)
    xy = str(first_correct)+str(other_correct)
    elf_codes.append(xy)

sum = 0
for elem in elf_codes:
    sum += int(elem)
    #print (elem)
print()
print(sum)